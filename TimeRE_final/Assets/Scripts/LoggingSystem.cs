...
public class LoggingSystem : MonoBehaviour {
    ...
    private static string LOGFILE_NAME_BASE_FRAMES = "_log_file_frames.csv";
    ...
    void Start () {
		...
		if(this.activeLogging)
		{
            ...
            // ### Frame LOGGING STARTS HERE ###

            

            // create file for this session using time prefix based on standard UTC time
            this.logFileFrames = LOGFILE_DIRECTORY
                + "/"
                + System.DateTime.UtcNow.ToString(LOGFILE_NAME_TIME_FORMAT)
                //+ System.DateTime.UtcNow.AddHours(2.0).ToString(LOGFILE_NAME_TIME_FORMAT)	// manually adjust time zone, e.g. + 2 UTC hours for summer time in location Stockholm/Sweden
                + LOGFILE_NAME_BASE_FRAMES;
            File.Create(this.logFileFrames);

            if (File.Exists(this.logFileFrames)) Debug.Log("[LoggingSystem] LogFile created at " + this.logFileFrames);
            else Debug.LogError("[LoggingSystem] Error creating LogFile");
    ...
    public void writeMessageToLogFrames(string message)
    {
        if (this.activeLogging)
        {
            if (File.Exists(this.logFileFrames))
            {
                TextWriter tw = new StreamWriter(this.logFileFrames, true);
                tw.WriteLine(message);
                tw.Close();
            }
        }
    ...
    public void writeHeadingsFrames(){
        writeMessageToLogFrames("ID" + ";" + "Frame" + ";" + "Framerate");
    }
